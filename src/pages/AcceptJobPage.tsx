    ...
    const processToken = async () => {
      ...
      try {
        ...
        if (payload.action === 'accept') {
          // Accept the job
          console.log('Accepting job for project:', payload.projectId, 'contractor:', payload.contractorId);
          const { data: updatedProject, error: updateError } = await supabase
            .from('projects')
            .update({ 
              assigned_contractor_id: payload.contractorId,
              status: 'matched'
            })
            .eq('id', payload.projectId)
            .is('assigned_contractor_id', null) // Only update if not already assigned
            .select('*, contractors(*)')
            .single()
          
          console.log('Update result:', updatedProject ? 'Success' : 'Failed', updateError || '');
          ...
          await supabase
            ...
            .eq('project_id', payload.projectId)
            .neq('contractor_id', payload.contractorId)

          setStatus('success')
          setMessage('Congratulations! You have successfully accepted this job.')

          // Send confirmation email to homeowner
          try {
            if (project.contact_email && updatedProject.contractors) {
              await resendEmailService.sendJobAcceptedConfirmation({
                homeownerEmail: project.contact_email,
                homeownerName: project.contact_name,
                contractorName: updatedProject.contractors.contact_name,
                companyName: updatedProject.contractors.company_name,
                projectDetails: {
          ...
          // Create notifications for job acceptance
          try {
            // Notify homeowner that contractor accepted
            if (project.user_id) {
              await supabase.from('notifications').insert({
                user_id: project.user_id,
                type: 'job_accepted',
                title: '✅ Contractor Assigned!',
                message: `A contractor has accepted your job and will contact you soon to schedule inspection.`,
                data: {
            ...
            // Notify contractor of successful acceptance
            ...
            if (contractorProfile?.user_id) {
              await supabase.from('notifications').insert({
                ...
                message: `You've been assigned the job at ${project.address}. Contact the homeowner within 24 hours.`,
                data: {
                  projectId: project.id,
            ...
            console.error('Error creating acceptance notifications:', notificationError)
          }

          // TODO: Move SMS notifications to server-side API endpoint
          // SMS confirmations should be sent via Supabase Edge Function
